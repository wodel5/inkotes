class FolegeVersion {
  ...
  FolegeVersion(this.major, this.minor, this.patch, [this.revision = 0])
    : assert(major >= 0 && major < 100),
      assert(minor >= 0 && minor < 100),
      assert(patch >= 0 && patch < 100),
      assert(revision >= 0 && revision < 10);

  factory FolegeVersion.fromName(String name) {
    final parts = name.split('.');
    assert(parts.length == 3);
    return FolegeVersion(
      int.parse(parts[0]),
      int.parse(parts[1]),
      int.parse(parts[2]),
    );
  ...
  factory FolegeVersion.fromNumber(int number) {
    ...
    return FolegeVersion(major, minor, patch, revision);
  }
  ...
  FolegeVersion bumpMajor() => FolegeVersion(major + 1, 0, 0);
  FolegeVersion bumpMinor() => FolegeVersion(major, minor + 1, 0);
  FolegeVersion bumpPatch() => FolegeVersion(major, minor, patch + 1);

  FolegeVersion copyWith({int? major, int? minor, int? patch, int? revision}) =>
      FolegeVersion(
        major ?? this.major,
        minor ?? this.minor,
        patch ?? this.patch,
