...
import 'package:foledge/components/settings/update_dialog.dart';
import 'package:foledge/data/prefs.dart';
import 'package:foledge/data/foledge_version.dart';
import 'package:foledge/data/version.dart' as version;

abstract class UpdateManager {
  static final log = Logger('UpdateManager');

  static final Uri versionUrl = Uri.parse(
    'https://raw.githubusercontent.com/foledge-notes/foledge/main/lib/data/version.dart',
  );
  static final Uri apiUrl = Uri.parse(
    'https://api.github.com/repos/foledge-notes/foledge/releases/latest',
  );
  ...
  static UpdateStatus getUpdateStatus(
    ...
    final currentVersion = FolegeVersion.fromNumber(
      currentVersionNumber,
    ).copyWith(revision: 0);
    final newestVersion = FolegeVersion.fromNumber(
      newestVersionNumber,
    ).copyWith(revision: 0);
    ...
    if (!stows.shouldAlwaysAlertForUpdates.value) {
      ...
      if (newestVersion.buildNumber - currentVersion.buildNumber <
          FolegeVersion.fromName('0.0.2').buildNumber) {
  ...
  static Future<String?> getChangelog({
    ...
    final url =
        'https://raw.githubusercontent.com/foledge-notes/foledge/main/'
